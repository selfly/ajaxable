...
class Ajaxable extends EventEmitter {
     ...
     * });
     */
    constructor(el, options = '', win = window) {
        super();
        if (!el) {
    ...
    send(el) {
        ...
        if (!subEl) {
            subEl = el.appendChild(this.win.document.createElement('button'));
            subEl.id = id;
    ...
    parseEl(el) {
        if (typeof el === 'string') {
            el = this.win.document.querySelectorAll(el);
        }
        if (!el.length ||
            el instanceof this.win.HTMLElement) {
            el = [el];
        }
        return el;
    }
    ...
    fetchData(el) {
        this.checkForm(el);
        let formData = new this.win.FormData(el);
        return formData;
    }
     ...
     */
    checkForm(el) {
        if (!el || !(el instanceof this.win.HTMLFormElement)) {
